...
def get_cosine_lr_func(lr_init, lr_final, lr_delay_steps=0, max_steps=1000000):
    """
    Continuous learning rate decay function with cosine annealing.
    The learning rate will:
    1. Stay constant at lr_init for the first lr_delay_steps
    2. Then decay following a cosine schedule from lr_init to lr_final
    3. Stay at lr_final after reaching the final learning rate
    
    Args:
        lr_init: float, initial learning rate
        lr_final: float, final learning rate
        lr_delay_steps: int, number of steps to maintain initial learning rate
        max_steps: int, total number of training steps
    
    Returns:
        function: A function that takes step as input and returns the learning rate
    """
    ...
    def helper(step):
        ...
        if step < lr_delay_steps:
            # Keep initial learning rate during delay steps
            return lr_init
            
        # Calculate progress after delay steps
        progress = np.clip((step - lr_delay_steps) / (max_steps - lr_delay_steps), 0, 1)
